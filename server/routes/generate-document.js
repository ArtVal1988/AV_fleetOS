...
const FIELD_CATALOG = [
  ...
  { key: 'client_name_resident', label: 'ПІБ — резидента (порожньо, якщо клієнт нерезидент)', get: ctx => ctx.client?.residency !== 'nonresident' ? (ctx.client?.name || ctx.booking.customer?.name || '') : '' },
  { key: 'client_name_nonresident', label: 'ПІБ — нерезидента (порожньо, якщо клієнт резидент)', get: ctx => ctx.client?.residency === 'nonresident' ? (ctx.client?.name || ctx.booking.customer?.name || '') : '' },
  { key: 'client_birthdate_resident', label: 'Дата народження — резидента (порожньо, якщо клієнт нерезидент)', get: ctx => ctx.client?.residency !== 'nonresident' ? fmtDateUk(ctx.client?.birthdate) : '' },
  { key: 'client_birthdate_nonresident', label: 'Дата народження — не резидента (порожньо, якщо клієнт резидент)', get: ctx => ctx.client?.residency === 'nonresident' ? fmtDateUk(ctx.client?.birthdate) : '' },
